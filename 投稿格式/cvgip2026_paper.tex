...
% Sub-cluster size table omitted for page-limit compliance;
% sizes: sub\_0--16 are 350, 63, 47, 19, 134, 575, 72, 270, 1014,
%         76, 126, 143, 620, 137, 178, 189, 7 (total 4,020).
...
The main limitation is intrinsic to purely unsupervised training: large pose variation within a single species can cause that species to split across multiple clusters, because appearance---not taxonomy---drives the grouping. The hierarchical sub-clustering stage mitigates the opposite failure (one cluster mixing species) but does not merge pose-induced splits. Without ground-truth labels we rely on label-free indices; ACC/NMI/ARI require an annotated validation subset, which we leave for future work. The hierarchical sub-clustering is also semi-automatic: default ensemble parameters tend to over-segment, so the final $17$ sub-clusters were obtained after a light manual merge-and-prune pass; automating this consolidation is a concrete direction for future work.

A further point concerns the role of the deep-clustering stage itself. Clustering frozen DINOv2 features with $k$-means already yields balanced groups (normalized size entropy $0.98$ vs.\ $0.97$ for our model). We nonetheless adopt DINOv2-based SeCu for three practical reasons: it is end-to-end, it learns multiple granularities jointly through its $K\!\in\!\{8,9,10\}$ heads, and its size constraint guarantees no cluster collapses (smallest/largest ratio $0.38$).

For future work, the most promising direction is semi-supervised fine-tuning using the clusters as pseudo-labels. An agronomist names a handful of groups; these supervise a lightweight classifier, closing the loop from zero-label discovery to deployable pest monitoring.
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.90\linewidth]{subcluster_example.png}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.90\linewidth]{tsne.png}
