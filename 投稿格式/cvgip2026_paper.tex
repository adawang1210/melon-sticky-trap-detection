...
Because the dataset has no ground-truth species labels, we evaluate the partition with three \emph{internal} validation indices that require no labels: the Silhouette coefficient (higher is better), the Davies--Bouldin index (DBI, lower is better), and the Calinski--Harabasz index (CHI, higher is better). All three are computed on the 128-d L2-normalized embedding produced by the trained projection head, with Silhouette under the cosine metric to match the normalized features.\footnote{Label-dependent scores (ACC, NMI, ARI under Hungarian assignment) are omitted because no ground-truth species labels are available; our inference script computes them automatically once a labeled validation subset is annotated.} Table~\ref{tab:metrics} compares backbones and losses under these indices. Our full model (DINOv2 $+$ size-mml) attains a Silhouette of $0.485$, DBI $1.13$, and CHI $1497.6$ over the $N\!=\!4{,}305$ crops, which form eight well-separated, non-empty clusters; the remaining rows are marked TBD pending the corresponding training runs.

\begin{table}[t]
\centering
\caption{Label-free clustering quality across backbones and losses. Silhouette (cosine)~$\uparrow$, Davies--Bouldin~$\downarrow$, Calinski--Harabasz~$\uparrow$. Bold = our full model; TBD rows await the corresponding training runs.}
\label{tab:metrics}
\small
\begin{tabular}{@{}llccc@{}}
\toprule
...
We design four ablations to isolate the contribution of each component, scored with the same label-free indices as Table~\ref{tab:metrics}. Blocks (a)--(c) are evaluated on the main 128-d embedding; block (d) concerns the sub-clustering dimensionality reduction and is therefore evaluated on a single coarse cluster in a fixed PCA reference space (so the three DR variants are directly comparable), which is on a different scale from (a)--(c). The $\Delta$ column reports the relative change in Silhouette over the stated baseline; rows not yet run are marked TBD.

\begin{table}[t]
\centering
\caption{Ablation study under label-free indices (Silhouette~$\uparrow$, DBI~$\downarrow$, CHI~$\uparrow$). $\Delta$ = relative change in Silhouette over the stated baseline. Block (d) is evaluated in a fixed sub-clustering reference space and is not comparable to (a)--(c).}
\label{tab:ablation}
\small
\begin{tabular}{@{}lcccc@{}}
\toprule
...
Full model              & \textbf{0.485} & \textbf{1.13} & \textbf{1497.6} & --- \\
...
DINOv2 (ours)           & \textbf{0.485} & \textbf{1.13} & \textbf{1497.6} & --- \\
\midrule
\multicolumn{5}{l}{\textit{(d) Sub-clustering DR (ref.\ space)}}\\
PCA-only                & TBD & --- & --- & TBD\% \\
UMAP-only               & TBD & --- & --- & TBD\% \\
PCA + UMAP (ours)       & TBD & --- & --- & --- \\
\bottomrule
