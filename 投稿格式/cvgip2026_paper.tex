...
\begin{table}[t]
\centering
\caption{Ablation under label-free indices (Silh.\,$\uparrow$, DBI\,$\downarrow$, CHI\,$\uparrow$). $\Delta$ = relative Silhouette change. Block~(d) reports clustering-space Silhouette on cluster~0 (sub-count in parentheses); not comparable to (a)--(c).}
\label{tab:ablation}
...
\midrule
\multicolumn{5}{l}{\textit{(a) Medoid re-estimation (mean$\,\pm\,$std, 3 seeds)}} \\
...
\begin{figure*}[t]
  ...
  \begin{tikzpicture}[
    font=\scriptsize,
    box/.style={draw, rounded corners, align=center, text width=1.85cm,
