...
class ImageDataset(Dataset):
    ...
    def _scan_files(self, folder_path):
        ...
        for root, dirs, files in os.walk(folder_path):
            # 找出目前資料夾中所有圖片
            current_files = [f for f in files if f.lower().endswith(valid_extensions)]
            ...
            for file in current_files:
                ...
                file_names.append(f_name)
                file_paths.append(full_path)
                # --- 關鍵修改：不要用資料夾名稱當標籤，通通設為 'Unknown' ---
                ground_truth_list.append('Unknown') 

        if not file_names:
            print(f"[Error] 在 {folder_path} 未找到任何有效的資料！")
            return [], [], [], {}
