...
parser.add_argument('--warm-up', default=15, type=int,
                    help='use new revised with SeCu')
parser.add_argument('--freeze-encoder', action='store_true', help='Freeze encoder when resuming')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.distributed:
        ...
        if args.gpu is not None:
            ...
            model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[args.gpu]) #add find_unused_parameters=True
        else:
            model.cuda()
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            print("=> loading encoder from checkpoint '{}'".format(args.resume))
            loc = f"cuda:{args.gpu}" if torch.cuda.is_available() else "cpu"
            checkpoint = torch.load(args.resume, map_location=loc)
            encoder_weights = {k: v for k, v in checkpoint['state_dict'].items() if 'encoder' in k}
            model.load_state_dict(encoder_weights, strict=False)
            # 如果指定了 --freeze-encoder，則凍結 encoder
            if getattr(args, 'freeze_encoder', False):
                for name, param in model.named_parameters():
                    if 'encoder' in name:
                        param.requires_grad = False
                print("✅ Encoder loaded and frozen. Clustering head has been reset.")
            else:
                print("✅ Encoder loaded. Clustering head has been reset.")

            # === 自動建立 optimizer 參數列表（在 resume 時更新） ===
            centers = []
            encoder = []
            for name, param in model.named_parameters():
                if 'center' in name and param.requires_grad:
                    centers.append(param)
                elif 'encoder' in name and param.requires_grad:
                    encoder.append(param)

            optimizer = torch.optim.SGD(
                [{"params": encoder, "lr": args.lr},
                {"params": centers, "lr": args.clr}],
                weight_decay=args.weight_decay,
                momentum=args.momentum
            )
    ...
    aug_2 = [
        transforms.RandomResizedCrop(crop_size, scale=(args.min_crop, 1.)),
        transforms.RandomApply([
            transforms.ColorJitter(0.3, 0.3, 0.15, 0.5)
        ], p=0.8),
        transforms.RandomGrayscale(p=0.2),
    ...
    train_dataset = secu.folder.NPYFolder(
        traindir,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, model, criterion, optimizer, epoch, args, scaler,writer)
...
def train(train_loader, model, criterion, optimizer, epoch, args, scaler,writer):
    ...
    # --- 新增：重新挑 medoid ---
    if args.use_medoid and epoch >= 25:           # 先跑 15 
        model.module.recompute_medoids(train_loader) #  
    pcenters = model.module.get_centers()          # 供 swapped prediction
    # 
    # ----------------------------------------------
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if i % args.print_freq == 0:
            ...
            for h in range(args.secu_num_head):
                writer.add_scalar(f'train/cluster{h}_max',model.module.counters[h].max().item(),global_step)
                writer.add_scalar(f'train/cluster{h}_min',model.module.counters[h].min().item(),global_step)
