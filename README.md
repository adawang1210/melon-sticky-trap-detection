...
| 6 | 370 | 8.6% | 0.936 |
| 7 | 346 | 8.0% | 0.942 |
| **合計 / 平均** | **4305** | **100%** | **0.943** |

> Cohesion = 群內樣本對群中心方向的平均 cosine 相似度（免標籤，越高代表群越緊）。

### 量化評估（無標籤內在指標）

資料無 ground-truth 物種標籤，故採用三種免標籤內在指標：**Silhouette**（cosine，↑）、**Davies–Bouldin (DBI)**（↓）、**Calinski–Harabasz (CHI)**（↑），皆在 128 維 L2-normalized embedding 上計算（N=4305）。
...
| **DINOv2 / size-mml（釋出模型）** | | **0.485** | **1.13** | **1497.6** |

> DINOv2 全面領先；DINOv2 的 size 與 size-mml 幾乎並列（MML 對最終模型為中性）。

**分群數 K 選擇**（DINOv2 多 head）
...
| 項目 | 設定 | Silhouette | 備註 |
|---|---|---|---|
| (a) Medoid（3 seeds） | 有 medoid | **0.486 ± 0.002** | baseline |
| | 無 medoid | 0.466 ± 0.009 | **+4.5%**，且變異更小 → medoid 有效且穩定 |
| (b) MML | 有 MML | 0.485 | — |
| | 無 MML | 0.487 | 移除反而略升 → MML 中性 |
| (c) Backbone（loss=size） | DINOv2 | 0.487 | baseline |
| | ViT-B | 0.421 | −14% |
| | ResNet-18 | 0.292 | −40% |
| (d) 子聚類降維（cluster_0） | PCA + UMAP | 0.339（12 子群） | cluster-space silhouette |
| | UMAP-only | 0.348（12 子群） | 與 PCA+UMAP 並列 |
| | PCA-only | 0.096（3 子群） | UMAP 是關鍵組件 |
